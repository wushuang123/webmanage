 ...
 * </p>
 *
 * @mongodb.driver.manual core/object-id IDGenerator
 */
public final class IDGeneratorOfMongo implements Comparable<IDGeneratorOfMongo>, Serializable {

	private static final long serialVersionUID = 3670079982654483072L;

	static final Logger LOGGER = LoggerFactory.getLogger("IDGenerator");
	 ...
	 * @return the new id
	 */
	public static IDGeneratorOfMongo get() {
		return new IDGeneratorOfMongo();
	}

	/**
	 * Checks if a string could be an {@code IDGenerator}.
	 *
	 * @param hexString
	 *            a potential IDGenerator as a String.
	...
	/**
	 * <p>
	 * Creates an IDGenerator using time, machine and inc values. The Java driver
	 * used to create all IDGenerators this way, but it does not match the
	 * <a href="http://docs.mongodb.org/manual/reference/object-id/">IDGenerator
	 ...
	 *
	 * <p>
	 * NOTE: This will not break any application that use IDGenerators. The 12-byte
	 ...
	 * @param inc
	 *            incremental value
	 * @return a new {@code IDGenerator} created from the given values
	 * @since 2.12.0
	 */
	public static IDGeneratorOfMongo createFromLegacyFormat(final int time, final int machine, final int inc) {
		return new IDGeneratorOfMongo(time, machine, inc);
	}

	/**
	 * Create a new object id.
	 */
	public IDGeneratorOfMongo() {
		this(new Date());
	}
	 ...
	 *            the date
	 */
	public IDGeneratorOfMongo(final Date date) {
	 ...
	 */
	public IDGeneratorOfMongo(final Date date, final int counter) {
	 ...
	 *             zero
	 */
	public IDGeneratorOfMongo(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
		this(dateToTimestampSeconds(date), machineIdentifier, processIdentifier, counter);
	}

	/**
	 * Creates an IDGenerator using the given time, machine identifier, process
	 * identifier, and counter.
	 *
	 * @param timestamp
	 *            the time in seconds
	 ...
	 *             zero
	 */
	public IDGeneratorOfMongo(final int timestamp, final int machineIdentifier, final short processIdentifier,
			final int counter) {
		this(timestamp, machineIdentifier, processIdentifier, counter, true);
	}

	private IDGeneratorOfMongo(final int timestamp, final int machineIdentifier, final short processIdentifier,
	 ...
	 *             IDGenerator
	 */
	public IDGeneratorOfMongo(final String hexString) {
	 ...
	 */
	public IDGeneratorOfMongo(final byte[] bytes) {
	 ...
	 *            incremental value
	 */
	IDGeneratorOfMongo(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
	 ...
	 * @since 3.4
	 */
	public IDGeneratorOfMongo(final ByteBuffer buffer) {
		...
		// order
		// and IDGenerator's are always in big-endian order.
	 ...
	 *
	 * @return a string representation of the IDGenerator in hexadecimal format
	 */
	...
	public boolean equals(final Object o) {
		...
		IDGeneratorOfMongo objectId = (IDGeneratorOfMongo) o;
	...
	@Override
	public int compareTo(final IDGeneratorOfMongo other) {
		if (other == null) {
	...
	/**
	 * @return a string representation of the IDGenerator in hexadecimal format
	 * @see IDGeneratorOfMongo#toHexString()
	...
	private static byte[] parseHexString(final String s) {
		if (!isValid(s)) {
			throw new IllegalArgumentException("invalid hexadecimal representation of an IDGenerator: [" + s + "]");
		}
	...
	final class Assertions {
		 ...
		 *             if value is null
		 */
		public <T> T notNull(final String name, final T value) {
			if (value == null) {
		 ...
		 */
		public void isTrue(final String name, final boolean condition) {
			if (!condition) {
		 ...
		 *             if the condition is false
		 */
		public void isTrueArgument(final String name, final boolean condition) {
			if (!condition) {
		...
		public <T> T convertToType(final Class<T> clazz, final Object value, final String errorMessage) {
